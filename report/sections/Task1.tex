...
\toprule
{\textbf{Attack Label}} & {\textbf{Count$_{train}$}} & {\textbf{Percentage$_{train}$}} & {\textbf{Count$_{test}$}} & {\textbf{Percentage$_{test}$}} & {\textbf{Binary label}} \\
\midrule
...
First, we verified whether the train or test sets contained infinite, NaN or duplicated samples: we did not identify any of them. Second, we dropped the columns we have already identified as only containing the value 0 (\texttt{urgent}, \texttt{num\_outbouhnd\_cmds}, \texttt{is\_host\_login}) in both sets, since they do not carry any useful information while increasing the dimensionality. Third, since the attack labels are words, we encoded them using the \texttt{LabelEncoder}.

\paragraph{Preprocessing of categorical features}
To handle categorical features, we first analyzed the frequency distribution of their unique values within the training set. To prevent feature explosion during encoding, we established an ``allowed list'' for each feature, keeping only the most frequent values and mapping all remaining ones to \texttt{other}. In particular, \texttt{service} has 15 different values; we kept the six most frequent ones (\texttt{http}, \texttt{private}, \texttt{smtp}, \texttt{domain\_u}, \texttt{other}, \texttt{ftp\_data}, \texttt{ecr\_i}) and we added all the other samples to \texttt{other} (that already existed in the column). \texttt{flag} has 11 different values; we kept the five most frequent ones (\texttt{SF}, \texttt{S0}, \texttt{REJ}, \texttt{RSTR}, \texttt{RSTO}) and we mapped all the others to \texttt{other}. \texttt{protocol\_type} only contains three different values, so we kept all of them. Following this dimensionality reduction, we applied One-Hot Encoding to transform these categories into numerical vectors: at this point, the datasets are characterized by 52 features.

\paragraph{Preprocessing of numerical features} First, we visualized the distribution of values for the numerical features: since different features contain outliers (e.g., \texttt{num\_compromized}), we decided to apply standardization (Z-score scaling). The \texttt{StandardScaler} was fit only on the training set to avoid data leakage, with the same transformation applied to the test set.
